...
def to_csv_file(filename: str, headers: list, rows: list, delimiter: str = ',', new_line: str = "\n") -> None:
    """Записывает данные в файл csv формата.

    :param filename: файл, в который запишутся данные
    ...
    with open(filename, 'w') as f:
        stringofinput = delimiter.join(headers) + new_line
        for row in rows:
            stringofinput += delimiter.join(row) + new_line
        f.write(stringofinput)
